...
4. **Install ADFR** *(not needed in this reproduction stack)*
   Upstream FLOWR used the [ADFR suite](https://ccsb.scripps.edu/adfr/downloads/) together
   with the bundled GenBench3D benchmark. Both were removed here because this repository is
   sampling-only; see `../../assets/provenance.yaml`.
...
Upstream also required `genbench3d_data.tar` for its GenBench3D evaluation. That benchmark is
not part of this sampling-only reproduction stack, so the download is not needed.
