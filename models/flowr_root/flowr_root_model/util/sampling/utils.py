...
def angle_distance(
    ...
    # Only iterate over indices covered by atom_decoder; statistics may
    # contain more atom-type bins than the vocabulary defines.
    valency_weight = torch.zeros(len(w1_per_type), device=w1_per_type.device)
    for i in range(min(len(w1_per_type), len(atom_decoder))):
        valency_weight[i] = (
