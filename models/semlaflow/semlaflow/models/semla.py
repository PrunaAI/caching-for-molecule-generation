...
class EquiInvDynamics(torch.nn.Module):
    ...
    def _validate_forward_inputs(self, edge_feats, cond_coords):
        """
        Validate edge features and self-conditioning inputs for dynamics forward passes.

        Parameters
        ----------
        edge_feats : torch.Tensor or None
            Per-edge features if edges are modeled.
        cond_coords : torch.Tensor or None
            Conditional coordinates for self-conditioning.

        Raises
        ------
        ValueError
            If edge or conditional inputs are inconsistent with module configuration.
        """
        if edge_feats is not None and self.d_edge is None:
            raise ValueError(
                "edge_feats was provided but the model was initialised with d_edge as None."
    ...
    def _prepare_coord_sets(self, coords, atom_mask, cond_coords):
        """
        Project coordinates into learnable coordinate sets and apply the node mask.

        Parameters
        ----------
        coords : torch.Tensor
            Input coordinates, shape ``[batch_size, n_atoms, 3]``.
        atom_mask : torch.Tensor
            Mask for real atoms, shape ``[batch_size, n_atoms]``.
        cond_coords : torch.Tensor or None
            Optional conditional coordinates for self-conditioning.

        Returns
        -------
        tuple of torch.Tensor
            ``(coords, atom_mask_expanded)`` where ``coords`` has shape
            ``[batch_size, n_coord_sets, n_atoms, 3]`` and ``atom_mask_expanded`` matches it.
        """
        coords = (
    ...
    def _apply_layer_range(
        ...
        """
        Run a contiguous slice of message-passing layers.

        Parameters
        ----------
        coords : torch.Tensor
            Coordinate sets, shape ``[batch_size, n_coord_sets, n_atoms, 3]``.
        inv_feats : torch.Tensor
            Node features, shape ``[batch_size, n_atoms, d_model]``.
        adj_matrix : torch.Tensor
            Adjacency matrix, shape ``[batch_size, n_atoms, n_atoms]``.
        atom_mask : torch.Tensor
            Expanded node mask, shape ``[batch_size, n_coord_sets, n_atoms]``.
        edge_feats : torch.Tensor or None
            Edge features if ``d_edge`` is set.
        start_idx : int
            First layer index (inclusive).
        end_idx : int
            Layer index after the last layer to run (exclusive).

        Returns
        -------
        tuple of torch.Tensor
            ``(coords, inv_feats, edge_feats)`` after the requested layers.
        """
        for i in range(start_idx, end_idx):
            out = self.layers[i](
                coords, inv_feats, adj_matrix, atom_mask, edge_feats=edge_feats
            )
            if len(out) == 2:
        ...
        return coords, inv_feats, edge_feats

    def _forward_tail(self, coords, inv_feats, atom_mask, edge_feats):
        """
        Apply the final feed-forward block, coordinate head, optional bond refinement, and norms.

        Parameters
        ----------
        coords : torch.Tensor
            Coordinate sets after all message-passing layers.
        inv_feats : torch.Tensor
            Node features after all message-passing layers.
        atom_mask : torch.Tensor
            Expanded node mask, shape ``[batch_size, n_coord_sets, n_atoms]``.
        edge_feats : torch.Tensor or None
            Edge features before refinement, if applicable.

        Returns
        -------
        tuple
            Same return convention as :meth:`forward` (two- or three-tuple).
        """
    ...
    def forward_prefix_layers(
        ...
        """
        Run coordinate preparation and the first ``num_prefix_layers`` message-passing layers.
        ...
        coords : torch.Tensor
            Input coordinates, shape ``[batch_size, n_atoms, 3]``.
        inv_feats : torch.Tensor
            Invariant atom features, shape ``[batch_size, n_atoms, d_model]``.
        adj_matrix : torch.Tensor
            Adjacency matrix, shape ``[batch_size, n_atoms, n_atoms]``.
        atom_mask : torch.Tensor
            Mask for real atoms, shape ``[batch_size, n_atoms]``.
        edge_feats : torch.Tensor or None
            Edge features if ``d_edge`` is set.
        cond_coords : torch.Tensor or None
            Conditional coordinates if self-conditioning is enabled.
        num_prefix_layers : int
            Number of initial message-passing layers to run.

        Returns
        -------
        tuple
            ``(coords, inv_feats, edge_feats, adj_matrix, atom_mask_expanded)`` intermediate state.
        """
        self._validate_forward_inputs(edge_feats, cond_coords)
        n_layers = len(self.layers)
        if num_prefix_layers < 1 or num_prefix_layers > n_layers:
            raise ValueError(
                f"num_prefix_layers must be in [1, {n_layers}], got {num_prefix_layers}."
            )

        coords, atom_mask = self._prepare_coord_sets(coords, atom_mask, cond_coords)
        coords, inv_feats, edge_feats = self._apply_layer_range(
            coords,
            inv_feats,
            adj_matrix,
            atom_mask,
        ...
        return coords, inv_feats, edge_feats, adj_matrix, atom_mask

    def forward_suffix_from_prefix(
        self,
        coords,
        inv_feats,
        edge_feats,
        ...
        """
        Continue from a prefix state: run remaining layers and the output tail.

        Parameters
        ----------
        coords : torch.Tensor
            Coordinate sets after the prefix layers.
        inv_feats : torch.Tensor
            Node features after the prefix layers.
        edge_feats : torch.Tensor or None
            Edge features after the prefix layers, if applicable.
        adj_matrix : torch.Tensor
            Adjacency matrix.
        atom_mask : torch.Tensor
            Expanded node mask.
        num_prefix_layers : int
            Number of layers already applied in the prefix (suffix starts at this index).

        Returns
        -------
        tuple
            Same return convention as :meth:`forward`.
        """
        n_layers = len(self.layers)
        if num_prefix_layers < 0 or num_prefix_layers > n_layers:
            raise ValueError(
                f"num_prefix_layers must be in [0, {n_layers}], got {num_prefix_layers}."
            )
        coords, inv_feats, edge_feats = self._apply_layer_range(
            coords,
            inv_feats,
        ...
        return self._forward_tail(coords, inv_feats, atom_mask, edge_feats)
...
class SemlaGenerator(MolecularGenerator):
    ...
    def _prepare_generator_inputs(
        ...
        """
        Build atom features, adjacency, edge projections, and mask for the dynamics stack.
        ...
        coords : torch.Tensor
            Input coordinates, shape ``[batch_size, n_atoms, 3]``.
        inv_feats : torch.Tensor
            Raw invariant features before projection and size embedding.
        edge_feats : torch.Tensor or None
            Bond / edge features.
        cond_coords : torch.Tensor or None
            Conditional coordinates for self-conditioning.
        cond_atomics : torch.Tensor or None
            Conditional atom logits for self-conditioning.
        cond_bonds : torch.Tensor or None
            Conditional bond logits for self-conditioning.
        atom_mask : torch.Tensor or None
            Node mask; if ``None``, defaults to all ones.

        Returns
        -------
        tuple
            ``(atom_feats, adj_matrix, edge_feats_proj, atom_mask)`` where ``edge_feats_proj``
            may be ``None`` when no edges are used.
        """
        if cond_coords is not None and not self.self_cond:
            raise ValueError(
                "cond_coords was provided but the model was initialised with self_cond as False."
        ...
        if edge_feats is not None:
            ...
            edge_feats_proj = self.edge_in_proj(edge_feats)

        return atom_feats, adj_matrix, edge_feats_proj, atom_mask

    def _heads_from_dynamics_out(self, pred_coords, pred_feats, pred_edges, atom_mask):
        """
        Apply coordinate zeroing and classification heads to dynamics outputs.
        ...
        pred_feats : torch.Tensor
            Predicted node features before classifier heads.
        pred_edges : torch.Tensor or None
            Predicted edge features before ``edge_out_proj``, if applicable.
        atom_mask : torch.Tensor
            Node mask, shape ``[batch_size, n_atoms]``.
    ...
    def compute_prefix_state(
        ...
        *,
        ...
        """
        Run embeddings and the first ``num_prefix_layers`` dynamics layers only.

        Returns a tuple of tensors suitable for periodic caching (Taylor / AB / etc.).
        ...
        coords : torch.Tensor
            Input coordinates, shape ``[batch_size, n_atoms, 3]``.
        inv_feats : torch.Tensor
            Invariant atom features, shape ``[batch_size, n_atoms, n_feats]``.
        edge_feats : torch.Tensor or None
            Edge features if ``d_edge`` is set.
        cond_coords : torch.Tensor or None
            Conditional coordinates if self-conditioning is enabled.
        cond_atomics : torch.Tensor or None
            Conditional atom logits if self-conditioning is enabled.
        cond_bonds : torch.Tensor or None
            Conditional bond logits if self-conditioning is enabled.
        atom_mask : torch.Tensor or None
            Node mask.
        num_prefix_layers : int
            Number of layers in :attr:`dynamics.layers` to include in the prefix.

        Returns
        -------
        tuple of torch.Tensor
            ``(coords_ms, inv_feats, edge_feats, adj_matrix, atom_mask_expanded)`` intermediate state.
        """
        atom_feats, adj_matrix, edge_feats_proj, atom_mask = self._prepare_generator_inputs(
            coords,
            inv_feats,
            edge_feats,
            cond_coords,
        ...
        return self.dynamics.forward_prefix_layers(
            coords,
            atom_feats,
            adj_matrix,
            atom_mask,
    ...
    def complete_forward_from_prefix(
        self,
        prefix_state,
        num_prefix_layers,
        atom_mask,
        ...
        """
        Run the remaining dynamics layers and classification heads from a cached prefix state.

        Parameters
        ----------
        prefix_state : tuple
            Output of :meth:`compute_prefix_state` (or a Taylor approximation thereof).
        num_prefix_layers : int
            Number of prefix layers already applied (must match ``compute_prefix_state``).
        atom_mask : torch.Tensor
            Node mask, shape ``[batch_size, n_atoms]``, used for ``zero_com`` and masking.
        ...
        tuple
            Same return convention as :meth:`forward`.
        """
        coords_ms, inv_feats, edge_feats, adj_matrix, atom_mask_exp = prefix_state
        out = self.dynamics.forward_suffix_from_prefix(
            coords_ms,
            inv_feats,
            edge_feats,
            adj_matrix,
        ...
        return self._heads_from_dynamics_out(
            pred_coords, pred_feats, pred_edges, atom_mask
        )
