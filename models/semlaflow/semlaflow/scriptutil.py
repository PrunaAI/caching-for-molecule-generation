...
def generate_molecules(
    ...
    """
    Run batched molecular generation on the test dataloader.
    ...
    dm : LightningDataModule
        Datamodule providing ``test_dataloader()``.
    steps : int
        Number of ODE integration steps.
    strategy : str
        Time grid: ``\"linear\"`` or ``\"log\"``.
    solver : str, optional
        ``\"euler\"`` (default), ``\"heun\"``, or ``\"dpmpp\"`` (DPM-Solver++; requires diffusers).
    stabilities : bool, optional
        If ``True``, also return per-molecule stability flags.
    gpu_id : int, optional
        CUDA device index.
    break_after_first : bool, optional
        If ``True``, stop after the first batch (debug).

    Returns
    -------
    tuple
        ``(molecules, outputs)`` or ``(molecules, outputs, stabilities)`` if
        ``stabilities`` is ``True``.

    Examples
    --------
    >>> # molecules, outputs = util.generate_molecules(
    >>> #     model, dm, 100, "log", solver="euler"
    >>> # )  # doctest: +SKIP
    """
    ...
    for batch in tqdm(test_dl):
        ...
        output = model._generate(
            batch, steps=steps, strategy=strategy, solver=solver
        )
        outputs.append(output)
        if break_after_first:
            return

    molecules = [model._generate_mols(output) for output in outputs]
    molecules = [mol for mol_list in molecules for mol in mol_list]

    if not stabilities:
        return molecules, outputs

    stabilities = [model._generate_stabilities(output) for output in outputs]
